...
```
Figmaの各セクション → 「PNGとしてコピー」→ プレビューで保存
    ↓ design/input/ にまとめて置く
...
Figmaの各セクションごとに以下を繰り返す:

1. Figmaでセクション内の **通常状態のフレーム** を右クリック
2. **「PNGとしてコピー」** を選択
3. **プレビュー.app** を開く → `Cmd + N`（クリップボードから新規作成）
4. `Cmd + S` → 保存先: **`design/input/`** フォルダ
5. ファイル名は **Figmaのセクション名そのまま** でOK（例: `1,2：ログイン.png`）
