...
- 通常アカウントでは担当施設割当、施設提供設定、ユーザー施設別設定で認可を判定する。
- 共有システム管理者アカウント（`accountType='SYSTEM_ADMIN'`）は1件のみ用意し、未削除の全施設・全 `feature_code`・全 `column_code` を利用可能とする。
- 共有システム管理者アカウントの監査ログは当該共有アカウントの `user_id` として記録し、実際の利用者個人は追跡しない。
...
- 役割:
  - 通常アカウントではログインユーザーに割り当てられた担当施設一覧を表示する
  - 共有システム管理者アカウントでは未削除の全施設を表示する
...
- `GET /auth/context?actingFacilityId=...`
  ...
  - 必要ならメニュー表示グループ情報
  - を返す
  - 共有システム管理者アカウントでは、指定施設が未削除であることを検証したうえで全 `feature_code` / 全 `column_code` を返す
...
- 役割:
  ...
  - カラム表示可否の判定
  - 実効権限一覧の組み立て
- `accountType='SYSTEM_ADMIN'` の共有システム管理者アカウントでは、対象施設が未削除である限り通常の担当施設・機能設定判定をバイパスして許可する
- 判定元テーブル:
...
- 少なくとも保持したい内容:
  ...
  - 変更後値
  - 更新日時
- 共有システム管理者アカウントによる操作は共有アカウントの `user_id` を更新者として残し、実際の利用者個人を識別する追加ログは持たない
