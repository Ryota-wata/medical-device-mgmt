...
- 共有システム管理者アカウント（`account_type='SYSTEM_ADMIN'`）は、本カタログの通常判定をバイパスし、未削除施設に対して全 `feature_code` / 全 `column_code` を利用可能とする。
...
| `external_estimate_view` | 他施設公開 / 見積データ | 施設グループ管理 / 共有データ種別 | 設定値保持用。`usage_context='EXTERNAL'` |
| `external_data_history_view` | 他施設公開 / データ履歴 | 施設グループ管理 / 共有データ種別 | 設定値保持用。`usage_context='EXTERNAL'` |
...
| グループ | 配下コード |
| --- | --- |
| `USER_ADMIN` | `user_list_view`, `user_edit`, `user_facility_assignment_edit`, `facility_group_list`, `facility_group_edit`, `external_estimate_view`, `external_data_history_view` |
