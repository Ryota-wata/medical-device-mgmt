...
| ログイン・パスワード再設定（固定導線） | ログイン画面、パスワード再設定画面を使う | `auth_login` | `feature_code` | `SYSTEM_FIXED`。権限設定画面には出さない |
| 施設選択（固定導線） | 担当施設から作業対象施設を選ぶ | `facility_select` | `feature_code` | `SYSTEM_FIXED`。権限設定画面には出さない |
...
| 資産一覧 / 各種申請・管理登録 | 資産一覧上の各種申請・管理登録ボタンを表示し実行できる | `original_application` | `feature_code` | 資産一覧起点 |
...
| 申請ステータス | ステータス参照モーダルを表示する | `application_status` | `feature_code` | 補助参照 |

### 4-4. 棚卸し

| 権限管理単位一覧 A列 | 画面上の表示・利用 | 対応コード | 種別 | 備考 |
| --- | --- | --- | --- | --- |
| 棚卸し（現場） | 現場棚卸し画面を表示する | `inventory_field` | `feature_code` | 画面利用 |
| 棚卸し（事務） | 棚卸完了、Excel出力、関連モーダルを表示し実行できる | `inventory_office` | `feature_code` | 1管理単位で扱う |
...
| リモデル購入管理 / 申請受付～見積登録 | リモデル購入管理画面を表示する | `remodel_purchase` | `feature_code` | 画面利用 |
| リモデル購入管理 / 発注登録～資産登録 | 発注から資産登録までを実行できる | `remodel_order` | `feature_code` | 操作系 |
| リモデル購入管理 / 検収登録 | 検収登録画面を表示し実行できる | `remodel_acceptance` | `feature_code` | 操作系 |
| 見積管理（リモデル） | リモデル見積管理画面を表示する | `remodel_quotation` | `feature_code` | 既存機能を維持 |
...
| DataLINK / SHIP表示列 | 通常編集リスト内で SHIP 列を表示する | `ship_column` | `column_code` | カラム制御 |
| 通常購入管理 / 申請受付～見積登録 | 通常購入管理画面を表示する | `normal_purchase` | `feature_code` | 画面利用 |
| 通常購入管理 / 発注登録～仮資産登録 | 発注から仮資産登録までを実行できる | `normal_order` | `feature_code` | 操作系 |
| 通常購入管理 / 検収登録 | 検収登録画面を表示し実行できる | `normal_acceptance` | `feature_code` | 操作系 |
| 通常購入管理 / 見積管理 | 見積管理画面を表示する | `normal_quotation` | `feature_code` | 画面利用 |
| 移動・廃棄管理 | 移動・廃棄管理タブを表示する | `transfer_disposal` | `feature_code` | 画面利用 |
| 修理管理 | 修理管理タブを表示する | `repair_management` | `feature_code` | 画面利用 |
| 保守契約管理 | 保守契約管理タブ、関連モーダルを表示する | `maintenance_contract` | `feature_code` | 1管理単位で扱う |
| 点検管理 | 点検管理タブ、関連モーダル、予定表出力を表示する | `inspection_management` | `feature_code` | 1管理単位で扱う |
| 定期点検実施 | 定期点検実施モーダルを表示し実行できる | `periodic_inspection` | `feature_code` | 既存機能を維持 |
| 貸出管理（タスク管理） | 貸出管理タブ、関連モーダル、エクスポートを表示する | `lending_management` | `feature_code` | 1管理単位で扱う |
...
| 権限管理単位一覧 A列 | 画面上の表示・利用 | 対応コード | 種別 | 備考 |
| --- | --- | --- | --- | --- |
| 資産マスタ / 一覧 | 資産マスタ一覧を表示する | `asset_master_list` | `feature_code` | 一覧参照 |
| 資産マスタ / 新規作成・編集 | 編集/新規作成モーダルを表示する | `asset_master_edit` | `feature_code` | 編集系 |
| 施設マスタ / 一覧 | 施設マスタ一覧を表示する | `facility_master_list` | `feature_code` | 一覧参照 |
...
| SHIP部署マスタ / 一覧 | SHIP部署マスタ一覧を表示する | `ship_dept_master_list` | `feature_code` | 一覧参照 |
| SHIP部署マスタ / 新規作成・編集 | 編集/新規作成モーダルを表示する | `ship_dept_master_edit` | `feature_code` | 編集系 |
| 個別部署マスタ / 一覧 | 個別部署マスタ一覧を表示する | `hospital_dept_master_list` | `feature_code` | 一覧参照 |
| 個別部署マスタ / 新規作成・編集 | 編集/新規作成モーダルを表示する | `hospital_dept_master_edit` | `feature_code` | 編集系 |
| 業者マスタ / 一覧 | 業者マスタ一覧を表示する | `vendor_master_list` | `feature_code` | 一覧参照 |
| 業者マスタ / 新規作成・編集 | 編集/新規作成モーダルを表示する | `vendor_master_edit` | `feature_code` | 編集系 |
| 施設グループ / 一覧 | 施設グループ一覧を表示する | `facility_group_list` | `feature_code` | 一覧参照 |
...
以下は、`taniguchi/docs/ロール整理.xlsx` の `権限管理単位一覧` シート A列を正本として採用した結果、1つの管理単位で複数要素を制御するコードである。
