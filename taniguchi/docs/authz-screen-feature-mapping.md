...
最新化の基準は 2026-05-16 時点の `権限管理単位一覧` シート A列であり、`auth_login` と `facility_select` だけはシート外の固定導線として別扱いにしている。
...
- Phase1では `normal_ship_request` / `lending_in_use_used` を `config_scope='FACILITY_USER'` として施設提供設定にも表示し、施設で OFF の場合はユーザー側が ON でも実効権限なしとする
...
- Phase1では `normal_ship_request` / `lending_in_use_used` をユーザー別設定候補に含める。ユーザー側で `lending_in_use_used` を ON にする場合も、同一担当施設の `lending_checkout` が ON であることを必須とする
