...
ステータス件数: `Fix 27件` / `作成済み 0件` / `未着手 0件` / `不要 4件`
...
| 26 | 2 | SHIP代理作業依頼 | 個別ファイルなし | 23a. `/quotation-data-box/ship-proxy-requests`、60. `/quotation-data-box/rfq-process` | `不要` | Phase1のFix成果物から除外し、Phase2対象として再整理する。Fix外の作成済み設計書は削除済み。RFQプロセスの `SHIPへ一括依頼` はNo.24/25の業者向け見積依頼一括送信であり、本機能のSHIP代理作業依頼とは別責務とする。依頼作成API本体、施設選択画面の `SHIP依頼一覧へ`、ユーザー管理の `ship_proxy_task` 設定、リモデル管理の `remodel_ship_request` はPhase2で再整理する |
