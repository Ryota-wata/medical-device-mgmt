...
ステータス件数: `Fix 29件` / `作成済み 0件` / `未着手 0件` / `不要 4件`

Phase件数: `Phase1 32件` / `Phase2 1件`

本一覧は、内部作業メモである[機能要件.md](../機能要件.md)の画面一覧を起点に、API設計書の作成対象を機能単位で管理するための内部台帳である。開発者向けAPI設計の正本は、各機能の`openapi.yaml`と`design.md`とし、本一覧および`機能要件.md`は連携物に含めない。
既存ファイルの羅列ではなく、「どの機能を、どの設計書で管理するか」を正として扱う。

## ステータス定義
- `Fix`: OpenAPI、補足MD、DB整合、ソース監査が完了し、開発者が連携物だけで実装可能な水準。Wordの最新同期は納品監査で別途確認する
- `作成済み`: OpenAPI＋補足MDは作成済み。整合レビューまたは追加修正余地あり。Word生成は必須ではない
- `未着手`: 対応するAPI設計書は未作成
...
| 10 | 1 | 資産台帳取込・マスタ突き合わせ | `Fix/API設計書_資産台帳取込.docx` | 12. `/asset-import`、13. `/asset-matching` | `Fix` | 固定資産台帳取込29カラム（A～AC）、型変換・非同期エラー規則、施設ロケーションIDの解決/未解決修正、台帳原本値DTO、見積金額・税項目、マスタ側候補/選択値、DB保持先を反映済み。 |
| 11 | 1 | データ突合 | `Fix/API設計書_データ突合.docx` | 14. `/data-matching`、33. `/data-matching/ledger`、34. `/data-matching/me-ledger` | `Fix` | 固定資産台帳取込で保持した29カラムの原本直前スナップショット、`asset_ledgers` 反映、元3項目と確定値の分離、`raw_data_json` 非再解析、`lock_version` による更新競合、QR紐付け、現有品調査写真引継ぎを反映済み。 |
...
| 23 | 1 | 編集リスト | `Fix/API設計書_編集リスト.docx` | 22. `/remodel-application` | `Fix` | 通常購入/リモデル双方で使う編集リスト本体の独立設計書。作成・選択モーダルは `list_type='PURCHASE'` / `REMODEL` で分離し、作成後の種別切替は行わない。編集リスト一覧・作成・削除、作業ロック取得/heartbeat/解除、明細取得、セル編集、一括編集、Data Link、見積DB Link、フリーカラム、表示カラム設定/ブックマーク、行順変更、行削除、廃棄/移動の申請正本作成（`REMODEL`限定）、編集リスト起点RFQグループ作成を扱う。資産詳細参照はNo.12、購入管理タブからの申請取り込みはNo.25、リモデル管理一覧・ダッシュボード・クローズはNo.24を正本とする。明細ソースは `BASE_ASSET` / `APPLICATION` / `MANUAL` / `QUOTATION` を区分し、インライン新規要望は申請正本を作成して即 `編集中` へ取り込む。廃棄/移動申請APIは `applications` / `application_assets` と申請履歴だけを作成し、廃棄依頼グループと `rfq_applications` はNo.27のグループ作成時に作成する。見積DB Linkは `rfq_id` / `quotation_id` / `quotation_item_id` / `edit_list_item_id` を正本キーとし、認可は `normal_edit_list` / `remodel_edit_list` を基本とし、RFQ作成・廃棄/移動申請作成では関連権限を追加判定する。正本要件・DB設計・APIレビュー反映済み |
| 24 | 1 | リモデル管理 | `Fix/API設計書_リモデル管理.docx` | 55. `/quotation-data-box/remodel-management`、55a. `/quotation-data-box/remodel-dashboard`、60. `/quotation-data-box/rfq-process`、24. `/quotation-data-box/ocr-confirm`、44. `/quotation-data-box/category-registration`、47. `/quotation-data-box/item-ai-matching`、50. `/quotation-data-box/price-allocation`、54. `/quotation-data-box/registration-confirm`、25. `/quotation-data-box/order-registration`、26. `/quotation-data-box/inspection-registration`、27. `/quotation-data-box/asset-provisional-registration`、27a. `/quotation-data-box/asset-registration` | `Fix` | 通常リモデルRFQ（`REMODEL/RFQ`）の一覧・見積・発注・検収・原本登録とリモデルクローズを扱う。リモデル編集リスト起点の廃棄は `DISPOSAL/RFQ` として `rfq_applications.edit_list_id` / `edit_list_item_id` から関連表示し、`management_type='REMODEL'` の廃棄グループや旧 `REMODEL/DISPOSAL` 承認ワークフローは新規作成・操作しない。廃棄の見積依頼、見積登録、発注、作業日、完了はNo.27へ委譲し、No.24は `remodelCloseImpact` と `asset_ledgers.status='廃棄済'` を用いてクローズ可否を判定する。編集リスト本体操作、廃棄申請正本作成、通常RFQ作成はNo.23、廃棄申請管理の後続処理はNo.27を正本とする。正本要件・DB設計・APIレビュー反映済み |
...
| 27 | 1 | 移動・廃棄管理 | `Fix/API設計書_移動・廃棄管理.docx` | 35. `/disposal-task`、45. `/quotation-data-box/disposal-management`、58. `/quotation-data-box/transfer-management` | `Fix` | 移動・廃棄管理のAPI設計書。資産一覧、修理不能、リモデル編集リスト等から起票済みの申請を受け、廃棄申請受付、`DISPOSAL/RFQ` の廃棄依頼グループ作成、STEP①〜④の見積依頼・見積登録・発注・作業日/納期・完了登録を扱う。リモデル起点かどうかは `rfq_applications.edit_list_id` / `edit_list_item_id` で判定し、完了時は起点にかかわらず対象資産を `廃棄済` に更新する。No.24はリモデル起点廃棄の表示・クローズ判定だけを行い、後続操作は本書へ委譲する。ファイル選択・プレビューではDB保存せず、確定ボタン押下時にドキュメントと入力項目を保存する。見積一覧表示・個別プレビュー、発注書の印刷/メール送信、見送り、リモデルクローズ影響情報、旧廃棄URLの正規化を含む。正本要件・DB設計・APIレビュー反映済み |
| 28 | 1 | 修理管理 | `Fix/API設計書_修理管理.docx` | 57. `/quotation-data-box/repair-requests`、62. `/repair-task` | `Fix` | 修理管理の申請受付一覧、院内/外部依頼振り分け、初回受付情報の固定、修理タスク進行、STEP1の依頼先・依頼内容保存、見積依頼完了、参考見積・発注登録用見積・追加見積の登録と表示・削除、発注前の修理タスク削除、発注登録、作業日登録、完了書類登録、タスク完了、修理不能時の廃棄申請接続を扱う。メニュー起点の修理申請起票（`/repair-request`、`repair_request_create`）はNo.6 修理申請へ切り出し、本設計書では起票済み修理申請の受付以降を扱う。修理管理タブと修理タスク操作は `repair_management` で認可する。申請受付一覧は `status='新規申請'` かつ修理区分未設定、修理タスク管理リストは修理区分設定済みを対象とし、外部依頼は `status='新規申請'` のままでもSTEP1の修理タスクとして扱う。受付判定の同一再送は無更新で既存結果を返し、異なる修理区分への変更は拒否する。導入業者・保守契約は初回受付時に登録済み資産の発注・保守契約情報からスナップショットし、STEP1リクエストでは受け取らない。手入力見積依頼先は `rfq_vendors` へ保存し、最初の有効行を主依頼先として一覧用業者を同期する。見積提出期限は主依頼先と分離し、有効な `SENT` / `REPLIED` 依頼先の最短 `due_on` を一覧用期限へ同期する。修理タスク一覧の期限列はステータス別に見積提出期限、引取日、納入予定日を返し、完了・却下は表示対象外とする。`current_vendor_contact` は主依頼先または採用見積依頼先の `rfq_vendors.phone` 由来の電話番号とし、メールアドレスを保存しない。保存ステータスは `REPAIR=新規申請/見積依頼済/見積登録済/発注済/納期確定/完了/却下` を正本とし、`検収登録` は旧データ参照時だけSTEP4へ読み替える。STEP2の見積登録では画面の添付ファイル、見積フェーズ、保存形式、業者、見積No.、見積日付、見積金額、勘定科目をすべて必須で保存し、見積明細配列は受け取らず修理対象と見積金額から `quotation_items` を1件サーバー生成する。発注登録では当該明細から `order_items` を作成する。修理区分は `IN_HOUSE` / `OUTSOURCED` を保存正本とし、修理不能から廃棄申請へ接続する場合は登録済み資産・未登録資産の両方を対象として `disposal_application_details.related_repair_application_id` に元修理申請IDを保持する。Phase1ではSTEP1の依頼送信操作で依頼先情報等をDB保存するが実メールは送信せず、STEP2のメール送信はAPI・DB更新・状態遷移を伴わない。修理発注は `orders.order_type='修理'`、発注書はREPAIR/RFQ所有とし、発注書送付項目はNULLで保存する。完了時は資産台帳・個体情報を更新せず、対象の貸出管理機器が `使用不可` の場合だけ排他制御下で `貸出可` に戻す。正本要件・DB設計・APIレビュー反映済み |
| 29 | 1 | 保守契約管理 | `Fix/API設計書_保守契約管理.docx` | 15. `/asset-search-result`（保守契約登録導線）、41. `/maintenance-quote-registration`、49. `/quotation-data-box/maintenance-contracts` | `Fix` | 保守契約管理のAPI設計書。資産一覧または保守契約管理タブからの保守契約作成、保守契約一覧、契約グループ詳細、見積依頼、申請見送り、見積登録、契約登録、保守登録、契約内容見直し、契約更新、点検管理連携までを1本で扱う。保守契約は `applications` ではなく `maintenance_contracts` / `maintenance_contract_assets` を正本とし、見積依頼は `rfqs.management_type='MAINTENANCE'` と `rfq_vendors` で扱う。保守契約ステータスは `見積依頼 / 見積依頼済 / 見積登録済 / 完了 / 申請見送り` を正本とし、画面の `currentStep` は表示値として算出する。タスク管理側の一覧は進行中契約と契約終了日を過ぎていない `完了` 契約を表示し、契約終了日を過ぎたレコードと `申請見送り` は除外する。一覧取得APIには検索・ステータス・契約種別・期限切れ表示切替用のクエリパラメータを定義せず、既定並び順は保守契約No.（画面上の申請No.）昇順とする。完了レコードの詳細モーダルは閲覧専用とし、契約更新は入力を求めず、元契約名に「（更新）」を付けた契約グループ名を自動生成し、契約種別・種別備考と未除外の対象資産を引き継いだ後継契約を `見積依頼` で作成する。契約内容見直しは完了かつ契約終了日を過ぎていないレコードに対して既存資産の除外または新規資産追加を扱い、契約ステータスは `完了` のまま、見直し後金額・理由・追加/除外資産を履歴保持する。契約変更ドキュメントは任意添付（PDFのみ）とする。STEP③明細登録では保証期間を扱わず、保守契約由来の点検管理連携はメーカー保守のみとし、院内定期点検・院内スポット点検・日常点検は点検管理側で登録する。 |
| 30 | 1 | 点検管理 | `Fix/API設計書_点検管理.docx` | 32a. `/periodic-inspection`、42. `/maker-maintenance-result`、46. `/quotation-data-box/inspection-requests` | `Fix` | 点検管理のAPI設計書。定期点検タスク一覧、日常点検設定行一覧、点検メニュー登録、資産一覧画面で選択した原本資産から起動する点検管理登録、日常点検設定変更・設定解除、日程調整、スキップ、定期点検実施、メーカー保守結果登録、点検予定表出力までを1本で扱う。点検管理登録では対象資産の大分類・中分類・品目まで一致するメニューのみ適用でき、同分類の全資産へ自動展開しない。定期点検は1資産に複数メニューを登録可能とし、同一資産・同一定期点検メニューの有効行重複を禁止する。スポット点検は点検周期を持たず、作成時は日程未定を許可し、Action の日程調整で点検予定日を設定する。スポット点検はスキップ対象外とする。日常点検は `inspection_type='日常点検'` の1資産1有効行として保持し、使用前/使用中/使用後メニューを変更・一部解除できる。点検ステータスは定期点検系のみ `点検月超過 / 点検週 / 点検月 / 点検1ヶ月前 / 点検Nヶ月前 / 点検日調整 / 点検実施中 / 点検完了 / 再点検` を正本とし、`点検日調整` は `メーカー保守` または `院内スポット点検` で許可する。日常点検行はステータス、点検周期、前回点検日、次回点検予定を `NULL` / 画面 `-` 表示とする。定期点検開始時はQR照合成功または実施開始APIで `START_INSPECTION` を実行し、`inspection_tasks.status='点検実施中'` へ更新する。一覧のPhase1絞り込みは点検日区分、点検種別、点検グループ名、貸出状況を正本とし、Category/大分類/中分類/品目は必須条件にしない。点検種別の表示ラベル `院内点検` / `メーカー点検` / `スポット点検` / `日常点検` は保存値 `院内定期点検` / `メーカー保守` / `院内スポット点検` / `日常点検` へ変換する。日常点検のオフライン準備とQR起点の日常点検実施はNo.4を正本とし、日常点検設定行はNo.4のPWAパッケージ取得で端末へ配信する |
