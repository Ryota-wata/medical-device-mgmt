...
$contractDocumentInputRows = @(
  @('documentCategory', 'string', '✓', '`CONTRACT` 固定'),
  @('documentType', 'string', '✓', '`契約書` / `その他`。画面の「その他（免責部品一覧など）」は `その他` として送信する')
) + ($documentInputRows | Where-Object { $_[0] -notin @('documentCategory', 'documentType') })

$quotationDocumentInputRows = @(
  @('documentCategory', 'string', '✓', '`QUOTATION` 固定'),
  @('documentType', 'string', '✓', '`見積書` 固定')
) + ($documentInputRows | Where-Object { $_[0] -notin @('documentCategory', 'documentType', 'storageFormat') })

$contractReviewDocumentInputRows = @(
  @('documentCategory', 'string', '✓', '`CONTRACT_REVIEW` 固定'),
  @('documentType', 'string', '✓', '`契約変更覚書` / `その他`')
) + ($documentInputRows | Where-Object { $_[0] -notin @('documentCategory', 'documentType') })
...
$spec = @{
  ...
  Sections = @(
    ...
    @{ Type = 'Paragraph'; Text = '保守契約管理タブには契約終了日を過ぎていない契約のみ表示し、契約終了済みレコードは一覧から除外する。完了レコードの詳細モーダルは閲覧専用とし、契約内容見直しは完了かつ契約終了日を過ぎていない場合のみ、契約更新は一覧に表示される完了レコードから実行する。' },
    ...
    @{ Type = 'Paragraph'; Text = '点検管理への連携は保守登録時に `maintenance_contract_assets` のメーカー保守条件を入力として、メーカー保守の `inspection_tasks` を作成または更新する。院内定期点検、院内スポット点検、日常点検は保守契約管理から作成しない。点検実施、日程調整、メーカー保守結果登録は No.30 点検管理APIの責務とする。' },
    ...
    @{ Type = 'Heading3'; Text = 'QuoteRequestPreviewInput' },
    @{ Type = 'Table'; Headers = @('フィールド', '型', '必須', '説明'); Rows = $quoteRequestPreviewInputRows },
    @{ Type = 'Heading3'; Text = 'PdfPreviewUrlResponse' },
    @{ Type = 'Table'; Headers = @('フィールド', '型', '必須', '説明'); Rows = $pdfPreviewUrlRows },
    ...
    @{ Type = 'EndpointBlocks'; Items = @(
      @{
        ...
        ProcessingLines = @(
          ...
          '`見積依頼` / `見積依頼済` / `見積登録済` と `完了` のうち、`contract_end_on IS NULL OR contract_end_on >= 業務日` の契約を返す',
          '契約終了日を過ぎたレコードと `申請見送り` は一覧対象外とする',
          '検索・ステータス・契約種別・期限切れ表示切替用のクエリパラメータは定義しない',
          '期限表示は契約終了日を基準に算出する',
          '既定並び順は `maintenance_contract_no ASC`、`maintenance_contract_id ASC` とする'
        )
      ...
      @{
        ...
        RequestSubtables = @(
          @{
            ...
            Rows = $quoteRequestVendorInputRows
          }
        )
        PermissionLines = $permissionLines
        ProcessingLines = @(
          ...
          'STEP①の受付部署名、受付担当者名、受付連絡先はリクエスト本文では受け取らず、認証コンテキストのログインユーザー情報から補完する',
          '`maintenance_contracts.reception_department_name` 未保存時のみ `users.section_name`、`reception_person_name` 未保存時のみ `users.name`、`reception_contact` 未保存時のみ `users.phone_number` を保存する。保存済みの受付部署スナップショットは再保存時に上書きしない',
        ...
        ResponseRows = @(
          @('rfqId', 'int64', '✓', '保守契約RFQ ID'),
          @('receptionDepartmentName', 'string', '-', '保存または保持した受付部署名'),
          @('receptionPersonName', 'string', '-', '保存または保持した受付担当者名'),
          @('receptionContact', 'string', '-', '保存または保持した受付連絡先'),
      ...
      @{
        ...
        RequestHeaders = @('フィールド', '型', '必須', '説明')
        RequestRows = @(
          @('-', '-', '-', 'リクエストボディなし。見積依頼書PDFの表示・印刷は `POST /maintenance-quote-registration/contracts/{maintenanceContractId}/quote-request/preview` で都度生成する')
        )
        PermissionLines = $permissionLines
        ProcessingLines = @(
          $workFacilityProcessingLine,
          ...
          '本APIでは見積依頼書PDFや送付文書ファイルを受け取らず、`application_documents` へ保存しない',
      ...
      @{
        ...
        ProcessingLines = @(
          ...
          'STEP①の受付部署表示は `maintenance_contracts` の保存済み受付部署情報を返す。未保存の場合は認証コンテキストのログインユーザー情報（`users.section_name` / `users.name` / `users.phone_number`）から表示用初期値を補完して返す',
          '対象資産は `excluded_flag` を含めて返し、明細登録画面では除外済み行を履歴として表示できるようにする',
          '見積、契約書、契約補助資料の登録済み一覧は `application_documents` から取得する。見積依頼書PDFは保存しないため `application_documents` から取得しない',
          '登録済み見積と登録済みドキュメントはメタデータのみ返し、S3オブジェクトキー、S3直接URL、認可済みプレビューURLは返さない。右ペインで表示する場合は個別のプレビューURL取得APIを呼び出す'
        )
      ...
      @{
        Title = '見積依頼書PDFプレビュー生成（/maintenance-quote-registration/contracts/{maintenanceContractId}/quote-request/preview）'
        Overview = 'STEP①右ペインの見積依頼書プレビューおよび印刷用に、画面入力内容と対象資産情報からPDFを都度生成する。生成したPDFは保存しない。'
        Method = 'POST'
        Path = '/maintenance-quote-registration/contracts/{maintenanceContractId}/quote-request/preview'
        Auth = '要（Bearer）'
        ParametersTitle = 'パスパラメータ'
        ParametersHeaders = @('パラメータ', 'In', '型', '必須', '説明')
        ParametersRows = @(
        ...
        RequestTitle = 'リクエストボディ'
        RequestHeaders = @('フィールド', '型', '必須', '説明')
        RequestRows = $quoteRequestPreviewInputRows
        RequestSubtables = @(
          @{
            Title = 'vendors要素（QuoteRequestVendorInput）'
            Headers = @('フィールド', '型', '必須', '説明')
        ...
        ProcessingLines = @(
          $workFacilityProcessingLine,
          '対象契約が作業対象施設に属することを確認する',
          '対象契約ヘッダー、契約対象資産、受付部署情報を取得する。受付部署情報が未保存の場合は認証コンテキストのログインユーザー情報から表示用初期値を補完する',
          'リクエストされた `vendorRequestComment` と `vendors` はPDF生成にのみ利用し、`rfqs`、`rfq_vendors`、`maintenance_contracts`、`application_documents` は更新しない',
          'PDFは `application/pdf` としてレスポンスボディに返し、Amazon S3およびDBへ保存しない',
          '右ペインの印刷アイコンは、このAPIで表示中のPDFをブラウザ印刷する'
        )
        ResponseTitle = 'レスポンス（200：application/pdf）'
        ResponseHeaders = @('フィールド', '型', '必須', '説明')
        ResponseRows = @(
          @('body', 'binary', '✓', '見積依頼書PDFバイナリ。Content-Type は `application/pdf`')
        )
        StatusRows = @(
          @('200', 'PDF生成成功', 'application/pdf'),
      ...
      @{
        ...
        Overview = 'STEP③の明細登録として、対象資産別のメーカー保守情報、契約単価を保存し、必要に応じて契約対象資産を追加する。保証期間は扱わない。'
        Method = 'PUT'
        ...
        ProcessingLines = @(
          ...
          '`inspectionType` は未設定または `メーカー保守` のみ許可する。院内定期点検、院内スポット点検、日常点検は本APIでは受け付けない',
          '保証期間はリクエスト項目として受け付けず、`maintenance_contract_assets` に保存しない',
      ...
      @{
        ...
        Overview = 'STEP②で発注登録用見積または参考見積のPDFを登録する。'
        Method = 'POST'
        ...
        RequestRows = @(
          ...
          @('payload.document', 'DocumentInput', '✓', '見積原本PDFメタデータ。`filePartName` で対応するファイルパートを指定する'),
          @('files', 'binary[]', '✓', '`payload.document.filePartName` で参照される見積原本PDFファイル本体')
        )
        RequestSubtables = @(
        ...
        ProcessingLines = @(
          ...
          '`payload.document.filePartName` が multipart のファイルパートに存在することを確認する',
          '見積原本はPDFのみ許可する。拡張子 `.pdf`、MIMEタイプ `application/pdf`、1ファイルあたり50MB（52,428,800 bytes）以下を必須とし、PDF以外は 400 (`MAINTENANCE_CONTRACT_FILE_INVALID_TYPE`)、50MB超過は 400 (`MAINTENANCE_CONTRACT_FILE_SIZE_EXCEEDED`) とする',
          '`payload.document.fileSize` が指定されている場合はファイル実体のサイズと一致することを確認し、保存時は実体サイズを `application_documents.file_size_bytes` に保存する。`contentHash` 指定時はアップロード実体と照合する',
          ...
          '見積原本PDF本体をAPI内でAmazon S3へPutObjectし、S3オブジェクトキーは `application-documents/facility-{facilityId}/{yyyy}/{mm}/{uploadUuid}.pdf` 形式で発行する。keyは保存場所識別子であり、`maintenanceContractId` や `quotationId` などの業務IDを含めない',
          ...
          '登録レスポンスでは見積PDFのプレビューURLを返さない。右ペイン表示時は `GET /maintenance-quote-registration/contracts/{maintenanceContractId}/quotations/{quotationId}/preview-url` を呼び出す',
        ...
        StatusRows = @(
          ...
          @('400', '見積フェーズ、金額、PDFメタデータまたはファイル本体が不正。PDF以外または50MB超過を含む', 'ErrorResponse'),
          @('404', '対象契約または業者が存在しない', 'ErrorResponse'),
      ...
      @{
        Title = '見積PDFプレビューURL取得（/maintenance-quote-registration/contracts/{maintenanceContractId}/quotations/{quotationId}/preview-url）'
        Overview = 'STEP②右ペインで登録済み見積一覧から選択された見積PDFを表示するため、認可済みプレビューURLを払い出す。'
        Method = 'GET'
        Path = '/maintenance-quote-registration/contracts/{maintenanceContractId}/quotations/{quotationId}/preview-url'
        Auth = '要（Bearer）'
        ParametersTitle = 'パスパラメータ'
        ParametersHeaders = @('パラメータ', 'In', '型', '必須', '説明')
        ParametersRows = @(
        ...
        ProcessingLines = @(
          $workFacilityProcessingLine,
          '対象契約が作業対象施設に属することを確認する',
          '対象見積が契約の保守契約RFQに紐づき、`quotations.deleted_at IS NULL` であることを確認する',
          '対象見積の `application_documents.owner_type=''QUOTATION''`、`document_category=''QUOTATION''`、`mime_type=''application/pdf''`、`deleted_at IS NULL` のPDFメタデータを取得する',
          'S3オブジェクトキー、S3バケット名、S3直接URLはレスポンスに含めず、短時間有効な認可済み `previewUrl` を発行する',
          '右ペインの印刷アイコンは、このURLで表示中のPDFをブラウザ印刷する。URL払い出し時に印刷履歴は作成しない'
        )
        ResponseTitle = 'レスポンス（200：PdfPreviewUrlResponse）'
        ResponseHeaders = @('フィールド', '型', '必須', '説明')
        ResponseRows = $pdfPreviewUrlRows
        StatusRows = @(
          @('200', 'プレビューURL払い出し成功', 'PdfPreviewUrlResponse'),
          @('404', '対象契約、見積、またはPDFが存在しない', 'ErrorResponse'),
          @('403', '実効 `maintenance_contract` なし', 'ErrorResponse')
        )
      },
      @{
      ...
      @{
        ...
        ProcessingLines = @(
          ...
          '論理削除後の見積は登録済み見積一覧とプレビューURL取得対象から外す。削除済み見積に対するプレビューURL取得は 404 とする',
      ...
      @{
        ...
        Overview = 'STEP③で契約書、その他契約補助資料などの契約関連PDFドキュメントを登録する。'
        Method = 'POST'
        ...
        RequestHeaders = @('フィールド', '型', '必須', '説明')
        RequestRows = @(
          @('payload.documents', 'ContractDocumentInput[]', '✓', '登録する契約関連PDFドキュメント。1件以上。各要素の `filePartName` で対応するファイルパートを指定する'),
          @('files', 'binary[]', '✓', '`payload.documents[].filePartName` で参照される契約関連PDFドキュメントのファイル本体')
        )
        RequestSubtables = @(
          @{ Title = 'payload.documents要素（ContractDocumentInput）'; Headers = @('フィールド', '型', '必須', '説明'); Rows = $contractDocumentInputRows }
        )
        PermissionLines = $permissionLines
        ProcessingLines = @(
          $workFacilityProcessingLine,
          '対象契約が `見積登録済` であることを確認する',
          'STEP③画面のドキュメント種別は `契約書` / `その他（免責部品一覧など）` の2択のみとし、細分種別選択は設けない',
          '`契約書` は `documentCategory=''CONTRACT''`、`documentType=''契約書''` として保存する',
          '`その他（免責部品一覧など）` も契約関連ドキュメントとして扱い、`documentCategory=''CONTRACT''`、`documentType=''その他''` として保存する',
          '`payload.documents[].filePartName` が multipart のファイルパートに存在することを確認する',
          '契約関連ドキュメントはPDFのみ許可する。拡張子 `.pdf`、MIMEタイプ `application/pdf`、1ファイルあたり50MB（52,428,800 bytes）以下を必須とし、PDF以外は 400 (`MAINTENANCE_CONTRACT_FILE_INVALID_TYPE`)、50MB超過は 400 (`MAINTENANCE_CONTRACT_FILE_SIZE_EXCEEDED`) とする',
          '`payload.documents[].fileSize` が指定されている場合はファイル実体のサイズと一致することを確認し、保存時は実体サイズを `application_documents.file_size_bytes` に保存する。`contentHash` 指定時はアップロード実体と照合する',
          '各PDFファイル本体をAPI内でAmazon S3へPutObjectし、S3オブジェクトキーは `application-documents/facility-{facilityId}/{yyyy}/{mm}/{uploadUuid}.pdf` 形式で発行する。keyは保存場所識別子であり、`maintenanceContractId` などの業務IDを含めない',
          ...
          '登録レスポンスでは契約ドキュメントPDFのプレビューURLを返さない。右ペイン表示時は `GET /maintenance-quote-registration/contracts/{maintenanceContractId}/documents/{documentId}/preview-url` を呼び出す',
        ...
        StatusRows = @(
          ...
          @('400', '文書メタデータまたはPDFファイル本体が不正。PDF以外、50MB超過、`documentCategory` が `CONTRACT` 以外、または `documentType` が `契約書` / `その他` 以外', 'ErrorResponse'),
          @('404', '対象契約が存在しない', 'ErrorResponse'),
          @('409', 'ステータス不正', 'ErrorResponse'),
          @('502', 'S3保存または保存後ロールバック失敗', 'ErrorResponse')
        )
      ...
      @{
        Title = '契約ドキュメントPDFプレビューURL取得（/maintenance-quote-registration/contracts/{maintenanceContractId}/documents/{documentId}/preview-url）'
        Overview = 'STEP③右ペインで登録済みドキュメント一覧から選択された契約書/その他PDFを表示するため、認可済みプレビューURLを払い出す。'
        Method = 'GET'
        Path = '/maintenance-quote-registration/contracts/{maintenanceContractId}/documents/{documentId}/preview-url'
        Auth = '要（Bearer）'
        ParametersTitle = 'パスパラメータ'
        ParametersHeaders = @('パラメータ', 'In', '型', '必須', '説明')
        ParametersRows = @(
        ...
        ProcessingLines = @(
          $workFacilityProcessingLine,
          '対象契約が作業対象施設に属することを確認する',
          '対象ドキュメントが契約の保守契約RFQに属する `owner_type=''RFQ''`、`document_category=''CONTRACT''`、`document_type in (''契約書'', ''その他'')`、`mime_type=''application/pdf''`、`deleted_at IS NULL` のPDFであることを確認する',
          'S3オブジェクトキー、S3バケット名、S3直接URLはレスポンスに含めず、短時間有効な認可済み `previewUrl` を発行する',
          '右ペインの印刷アイコンは、このURLで表示中のPDFをブラウザ印刷する。URL払い出し時に印刷履歴は作成しない'
        )
        ResponseTitle = 'レスポンス（200：PdfPreviewUrlResponse）'
        ...
        StatusRows = @(
          @('200', 'プレビューURL払い出し成功', 'PdfPreviewUrlResponse'),
          @('404', '対象契約またはPDFドキュメントが存在しない', 'ErrorResponse'),
          @('403', '実効 `maintenance_contract` なし', 'ErrorResponse')
        )
      },
      @{
        ...
        ProcessingLines = @(
          ...
          '論理削除後のドキュメントは登録済みドキュメント一覧とプレビューURL取得対象から外す。削除済みドキュメントに対するプレビューURL取得は 404 とする',
          'S3実体は即時DeleteObjectせず、同一S3オブジェクトキーを参照する有効メタデータがなくなったことと保存期間を確認するS3ライフサイクルまたは後続クリーンアップで扱う'
        )
        ResponseTitle = 'レスポンス（204：なし）'
        ResponseHeaders = @('フィールド', '型', '必須', '説明')
      ...
      @{
        ...
        ProcessingLines = @(
          ...
          '`application_documents.owner_type=''RFQ''` かつ `document_category=''CONTRACT''` かつ `document_type in (''契約書'', ''その他'')` の契約書または契約補助資料が1件以上あることを確認する',
          '`inspection_type=''メーカー保守''` が設定された対象資産について、同一契約由来のメーカー保守タスクを `inspection_tasks` に作成または更新する。院内定期点検、院内スポット点検、日常点検は作成しない',
          '作成または更新するメーカー保守タスクは `next_inspection_on=NULL`、初期 `status=''点検日調整''` とする。保証期間や契約開始日から初回予定日は算出しない',
          '別契約由来で同一資産の有効なメーカー保守タスクがある場合は競合として扱い、保守登録を中断する',
      ...
      @{
        ...
        Overview = '完了かつ契約終了日を過ぎていない保守契約に対して、既存資産の除外、新規資産追加、見直し後契約金額、理由を履歴登録し、必要に応じて契約変更文書を添付する。'
        Method = 'POST'
        ...
        RequestRows = @(
          ...
          @('payload.documents', 'DocumentInput[]', '-', '任意添付する契約変更覚書等のPDF文書。添付する場合は1件以上とし、各要素の `filePartName` で対応するファイルパートを指定する'),
          @('files', 'binary[]', '-', '`payload.documents[].filePartName` で参照される契約変更PDF文書のファイル本体。文書を添付しない場合は省略する')
        )
        RequestSubtables = @(
          @{ Title = 'payload.documents要素（DocumentInput）'; Headers = @('フィールド', '型', '必須', '説明'); Rows = $contractReviewDocumentInputRows }
        )
        PermissionLines = $permissionLines
        ProcessingLines = @(
          $workFacilityProcessingLine,
          ...
          '`payload.removeMaintenanceContractAssetIds` と `payload.addAssetLedgerIds` の各配列内に重複がないことを確認する',
          '追加対象資産が作業対象施設に属し、`excluded_flag` にかかわらず同一契約内に同じ `asset_ledger_id` が存在しないことを確認する',
          ...
          '`payload.documents` と `files` は両方省略できる。添付する場合は両方を指定し、文書メタデータとファイル本体を `filePartName` で1対1に対応させる。片方のみ指定した場合は400を返す',
          '添付がある場合のみ、`payload.documents[].documentCategory` は `CONTRACT_REVIEW`、`documentType` は `契約変更覚書` / `その他` を許可する',
          '添付がある場合のみ、`payload.documents[].filePartName` が multipart のファイルパートに存在することを確認し、拡張子 `.pdf`、MIMEタイプ `application/pdf`、1ファイル50MB以下、`contentHash` を検証する',
          '添付がある場合のみ契約変更文書ファイル本体をAPI内でAmazon S3へPutObjectし、S3オブジェクトキーは `application-documents/facility-{facilityId}/{yyyy}/{mm}/{uploadUuid}.pdf` 形式で発行する。keyは保存場所識別子であり、`maintenanceContractId` や `maintenanceContractReviewId` などの業務IDを含めない',
          '添付がある場合のみ、契約変更文書を `application_documents.owner_type=''MAINTENANCE_CONTRACT_REVIEW''`、`maintenance_contract_review_id=作成した見直しID`、`document_category=''CONTRACT_REVIEW''`、`document_type`、`file_name`、`file_path=S3オブジェクトキー`、`mime_type`、`file_size_bytes`、`content_hash`、`storage_format`、`uploaded_by_user_id`、`uploaded_at` として保存する。S3バケット名やHTTPS URLはDBへ保存しない',
          '添付がある場合、Amazon S3保存後に見直し履歴、資産追加/除外、金額更新、文書メタデータ保存のいずれかへ失敗したときは、保存済みS3オブジェクトをDeleteObjectで破棄する。破棄に失敗した場合は 502 (`MAINTENANCE_CONTRACT_FILE_502_S3_WRITE_FAILED`) を返却し、再試行可能な運用ログを残す',
      ...
      @{
        ...
        Overview = '完了かつ契約終了日を過ぎていない契約から後継の保守契約グループを作成し、`見積依頼` から再開する。'
        Method = 'POST'
        ...
        ProcessingLines = @(
          $workFacilityProcessingLine,
          '更新元契約が `完了` かつ `contract_end_on >= 業務日` であることを確認する',
          '更新元契約を再オープンせず、`maintenance_contract_no` を採番して新しい `maintenance_contracts` を `contract_group_name=更新元契約名 + ''（更新）''`、`maintenance_type=更新元値`、`maintenance_type_note=更新元値`、`status=''見積依頼''`、`remote_maintenance_available=false`、`on_call_support=false`、`last_status_changed_at=現在日時` で作成する',
          '新契約の `renewal_source_maintenance_contract_id` に更新元契約IDを保持する',
          '契約検討開始日、コメント、受付部署、決済No.、契約日、契約業者、契約金額、契約期間は引き継がず、後継契約では未設定とする',
          '見積依頼先、登録済み見積、採用見積、契約業者、契約金額、契約期間、契約書・添付ドキュメント、点検情報、契約単価は複製しない',
          '更新元契約の `excluded_flag=true` の対象資産は複製対象から除外する',
          '更新元契約の未除外資産と同じ `asset_ledger_id` で後継契約の `maintenance_contract_assets` を `excluded_flag=false` として作成する。部署情報・商品情報は `asset_ledgers` から再取得し、点検情報と契約単価は初期化する'
        )
        ...
        StatusRows = @(
          ...
          @('409', '更新元契約が完了ではない、契約終了日を過ぎている、または複製対象資産が0件', 'ErrorResponse')
        )
      }
    ) },
