    ...
    @{ Type = 'EndpointBlocks'; Items = @(
      ...
      @{
        ...
        ProcessingLines = @(
          ...
          '発注書ファイル本体をAPI内でAmazon S3へPutObjectし、S3オブジェクトキーは `application-documents/facility-{facilityId}/{yyyy}/{mm}/{uploadUuid}.{ext}` 形式で発行する。keyは保存場所識別子であり、`disposalTaskId` や `orderId` などの業務IDを含めない',
      ...
      @{
        ...
        ProcessingLines = @(
          ...
          '完了証跡ファイル本体をAPI内でAmazon S3へPutObjectし、S3オブジェクトキーは `application-documents/facility-{facilityId}/{yyyy}/{mm}/{uploadUuid}.{ext}` 形式で発行する。keyは保存場所識別子であり、`disposalTaskId` などの業務IDを含めない',
