@{
  ...
  Sections = @(
    ...
    @{ Type = 'Table'; Headers = @('入力項目', '候補集合', '保存先', '保存ルール'); Rows = @(
      @('`facilityAssignments[*].enabledFeatureCodes`', '`feature_catalogs.config_scope=''FACILITY_USER''` かつ対象施設の `facility_feature_settings.is_enabled=true` の `feature_code`。`auth_login` / `facility_select` は候補に含めない。Phase1では `normal_ship_request` / `lending_in_use_used` は候補に含める', '`user_facility_feature_settings`', '候補集合の各 `feature_code` を現在値として管理し、リクエストに含むコードを `is_enabled=true`、含まないコードを `false` として作成・更新する。`lending_in_use_used` を ON にする場合は同じ担当施設の `lending_checkout` も ON にする'),
    ...
    @{ Type = 'EndpointBlocks'; Items = @(
      ...
      @{
        ...
        ProcessingLines = @(
          ...
          '`enabledFeatureCodes` は、対象施設の `availableFeatureCodes`（`feature_catalogs.config_scope=''FACILITY_USER''` かつ `facility_feature_settings.is_enabled=true` の `feature_code`）の部分集合でなければならない。`auth_login` と `facility_select` を含む `config_scope=''SYSTEM_FIXED''` の `feature_code` は受け付けない。Phase1では `normal_ship_request` / `lending_in_use_used` を `FACILITY_USER` として受け付ける',
      ...
      @{
        ...
        ProcessingLines = @(
          ...
          '`user_facility_assignments.assignment_type` は `defaultFacilityId` の施設を `PRIMARY`、それ以外を `SECONDARY` として内部導出する',
          '`enabledFeatureCodes` は、対象施設の `availableFeatureCodes` の部分集合でなければならず、`auth_login` と `facility_select` を含む `config_scope=''SYSTEM_FIXED''` の `feature_code` は受け付けない。Phase1では `normal_ship_request` / `lending_in_use_used` を `FACILITY_USER` として受け付ける',
          '`enabledFeatureCodes` に `lending_in_use_used` を含める場合は、同じ担当施設の `enabledFeatureCodes` に `lending_checkout` も含めなければならない。含まれない場合は 409 (`FACILITY_PERMISSION_SCOPE_CONFLICT`) とする',
