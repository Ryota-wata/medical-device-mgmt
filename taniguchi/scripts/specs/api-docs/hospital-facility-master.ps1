@{
  ...
  Sections = @(
    ...
    @{ Type = 'Paragraph'; Text = '機能コードは `hospital_facility_master` を用いる。現行の `role_permissions` では一覧・候補・出力は `R` 以上、インポート・新規作成・更新・削除は `W` 以上を要求する前提で設計する。' },
    @{ Type = 'Table'; Headers = @('処理', '機能権限コード', '必要レベル', '説明'); Rows = @(
      @('施設候補取得 / SHIP候補取得 / 一覧取得 / エクスポート / テンプレート取得', 'hospital_facility_master', 'R以上', '閲覧系処理'),
      @('インポートプレビュー / インポート実行 / 新規作成 / 更新 / 削除', 'hospital_facility_master', 'W以上', '変更系処理')
    ) },
    @{ Type = 'Heading2'; Text = '施設スコープ仕様' },
    @{ Type = 'Bullets'; Items = @(
      '施設候補・一覧・エクスポート・更新対象は、認可済み施設スコープ内に限定する',
      '一覧取得時に `facilityId` 未指定で固定施設も導出できない場合は 400 を返却する',
      '固定施設が導出できる利用者は、画面上の施設選択を省略できる'
    ) },
    ...
    @{ Type = 'EndpointBlocks'; Items = @(
      ...
      @{
        ...
        ParametersHeaders = @('パラメータ', 'In', '型', '必須', '説明')
        ParametersRows = @(
          @('facilityId', 'query', 'int64', '条件付き', '選択施設ID。固定施設が導出できる場合は省略可'),
      ...
      @{
        ...
        ParametersHeaders = @('パラメータ', 'In', '型', '必須', '説明')
        ParametersRows = @(
          @('facilityId', 'query', 'int64', '条件付き', '選択施設ID'),
          @('shipDivisionName', 'query', 'string', '-', '共通マスタ-部門の部分一致条件'),
          @('shipDepartmentName', 'query', 'string', '-', '共通マスタ-部署の部分一致条件')
        )
        PermissionLines = @(
