...
$endpointSpecs = @(
  ...
  @{
    ...
    ProcessingLines = @(
      ...
      '固定資産台帳取込では、後掲の「固定資産台帳Excel正式取込カラム」29項目（A～AC）を必須ヘッダーとする。ヘッダー比較時はセル内改行、全角スペース、半角スペースを除去し、正規化後の名称で照合する',
      '正式取込対象29カラムが不足する場合は初期取込を失敗させ、`asset_import_jobs.status=FAILED`、`error_message=FIXED_ASSET_COLUMN_MISSING: columns={カラム名}` を保存する。29カラム以外の余剰列は無視する',
      'AI分類・推薦の入力に必要な `品目名（元）` / `メーカー名（元）` / `型式（元）` が空の場合は代替カラムで補完せず、初期取込を失敗させ、`error_message=FIXED_ASSET_REQUIRED_VALUE_MISSING: row={行番号}, fields={項目名}` を保存する',
      ...
      '正式取込対象29カラムはすべて `asset_import_rows.raw_data_json` と型付きの `parsed_*` に保持する。後続処理は `raw_data_json` を再解析せず、`parsed_*` をデータ突合と原本確定への引継ぎ元として参照する',
      '固定資産台帳のA列 `施設ロケーションID` は、Excel入力値を `parsed_facility_location_id` に保持したうえで、対象ジョブの `facility_id` に属する有効な `facility_locations.facility_location_id` と完全一致する場合だけ `facility_location_id` に設定する。解決できない値は取込を失敗させず `facility_location_id=NULL` とし、突き合わせ画面で未解決として修正対象にする',
      ...
      '`ABC` 判定行のみ PoC1 方針の 3-gram コサイン類似度で AI 推薦候補を算出する。台帳側は `parsed_original_asset_name` / `parsed_original_manufacturer_name` / `parsed_original_model_name` を比較元とし、別名の非「元」項目を代替入力にはしない。3項目の全順列と括弧除去有無の2パターンで最大スコアを採用する',
  ...
  @{
    ...
    ProcessingLines = @(
      ...
      '再取込時も `POST /asset-import/jobs` と同じ正式29カラム検証、`raw_data_json` / `parsed_*` 展開、施設ロケーション解決、AI分類・AI推薦ルールを適用する。分類入力は `product_name:{parsed_original_asset_name}, spec:{parsed_original_model_name}` とし、`parsed_original_manufacturer_name` は推薦候補抽出の比較元にのみ利用する。非「元」項目を代替入力にはしない。本システムでは Model B は利用しない。固定資産台帳取込では値引き文字列判定を行わず、AI推薦は `ABC` 行のみ JMDN由来候補と全施設の原本資産台帳由来候補を同じ候補集合として比較する',
  ...
  @{
    ...
    ProcessingLines = @(
      ...
      '施設ロケーションの候補は `READY_FOR_MATCHING` の場合のみ対象ジョブの `facility_id` に属する有効な `facility_locations` から取得し、それ以外は空配列とする。Excel入力値の文字列は候補生成元にせず、解決済み候補との照合に利用する'
    )
    ...
    ResponseSubtables = @(
      ...
      @{
        ...
        Headers = @('フィールド', '型', '必須', '説明')
        Rows = @(
          @('facilityLocationOptions', 'FacilityLocationSummary[]', '✓', '対象施設の個別部署マスタに登録された施設ロケーション候補')
        )
      }
    )
    StatusRows = @(
  ...
  @{
    ...
    Overview = '取込結果一覧、AI推薦、保存済み選択値、施設ロケーションの解決状態を取得する。施設ロケーションID/Category/大分類/中分類/品目で絞り込み可能とし、既定では未確定行のみ返却する。AI分類結果はサーバー内部の推薦制御値であり、画面表示用レスポンスには含めない。'
    Method = 'GET'
    ...
    ParametersRows = @(
      ...
      @('facilityLocationId', 'query', 'int64', '-', '解決済み施設ロケーションIDの一致条件'),
    ...
    ProcessingLines = @(
      ...
      '施設ロケーションID/Category/大分類/中分類/品目は AND 条件で絞り込む。施設ロケーション未解決行は `facility_location_id IS NULL` として返却し、行レスポンスで解決状態を判定できるようにする',
      ...
      '現在の画面 DTO では施設ロケーション、品目名（元）/メーカー名（元）/型式（元）/数量/単位の表示に必要な項目を返却する。正式取込対象29カラムのその他項目も `asset_import_rows` の型付き `parsed_*` と `raw_data_json` に保持し、データ突合・原本確定では `parsed_*` を参照する',
      '画面要件上ページングは定義しない'
    )
    ...
    ResponseSubtables = @(
      @{
        ...
        Rows = @(
          ...
          @('rowNo', 'int32', '✓', '行番号'),
          @('parsedFacilityLocationId', 'string', '-', '施設ロケーションID（Excel入力原文）。`asset_import_rows.parsed_facility_location_id` を返す'),
          @('facilityLocationId', 'int64|null', '-', '解決済み施設ロケーションID。解決できない場合は null'),
          @('facilityLocation', 'FacilityLocationSummary|null', '✓', '解決済み施設ロケーションの表示値。未解決の場合は null'),
      ...
      @{
        Title = 'FacilityLocationSummary'
        Headers = @('フィールド', '型', '必須', '説明')
        Rows = @(
          @('facilityLocationId', 'int64', '✓', '`facility_locations.facility_location_id`'),
          @('building', 'string', '-', '棟。個別部署マスタの表示値'),
          @('floor', 'string', '-', '階。個別部署マスタの表示値'),
          @('departmentName', 'string', '-', '部門名。個別部署マスタの表示値'),
          @('sectionName', 'string', '-', '部署名。個別部署マスタの表示値'),
          @('roomName', 'string', '-', '室名。個別部署マスタの表示値')
        )
      }
    )
    StatusRows = @(
  ...
  @{
    ...
    Overview = 'Category / 大分類 / 中分類 / 品目 / メーカー / 型式 / 施設ロケーションの検索候補を取得する。資産階層は親階層に応じて絞り込み、施設ロケーションは対象ジョブの施設に属する個別部署マスタから取得する。'
    Method = 'GET'
    ...
    ParametersHeaders = @('パラメータ', 'In', '型', '必須', '説明')
    ParametersRows = @(
      @('assetImportJobId', 'query', 'int64', '✓', '対象ジョブID。`FACILITY_LOCATION` の場合はジョブの `facility_id` で候補を絞り込む'),
      @('field', 'query', 'string', '✓', '取得対象。`CATEGORY` / `LARGE_CLASS` / `MEDIUM_CLASS` / `ITEM` / `MANUFACTURER` / `MODEL` / `FACILITY_LOCATION`'),
    ...
    ProcessingLines = @(
      '対象ジョブの `facility_id` が Bearer トークン上の作業対象施設IDと一致し、`facilities.deleted_at IS NULL` の未削除施設であることを検証する',
      '`FACILITY_LOCATION` の候補は対象ジョブの `facility_id` に属し、`facility_locations.deleted_at IS NULL` の有効な個別部署マスタから取得する。候補の5項目（棟/階/部門名/部署名/室名）は表示用に返却する',
      'その他の候補は指定した階層に応じたマスタテーブルから取得する',
    ...
    ResponseSubtables = @(
      @{
        ...
        Rows = @(
          @('id', 'int64', '✓', '候補ID'),
          @('name', 'string', '✓', '候補表示名。施設ロケーションでは5項目を連結した表示名'),
          @('building', 'string', '-', '`FACILITY_LOCATION` の棟'),
          @('floor', 'string', '-', '`FACILITY_LOCATION` の階'),
          @('departmentName', 'string', '-', '`FACILITY_LOCATION` の部門名'),
          @('sectionName', 'string', '-', '`FACILITY_LOCATION` の部署名'),
          @('roomName', 'string', '-', '`FACILITY_LOCATION` の室名')
        )
      }
    )
    StatusRows = @(
  ...
  @{
    ...
    RequestRows = @(
      ...
      @('facilityLocationId', 'int64|null', '-', '施設ロケーション選択値。対象施設の `facility_locations.facility_location_id` を指定する。未解決状態で保存する場合は null'),
    ...
    ProcessingLines = @(
      ...
      '`facilityLocationId` が指定された場合は、`facility_locations` に存在し、論理削除されておらず、対象ジョブの `facility_id` と一致することを検証する。検証に失敗した場合は元の `parsed_facility_location_id` を保持したまま `facility_location_id` は更新しない',
      'リクエストに `facilityLocationId` が含まれない場合は既存の `facility_location_id` を維持し、明示的に null を指定した場合だけ未解決状態へ戻す',
      '`facilityLocationId=null` の保存は未解決状態への変更として許可するが、`isConfirmed=true` の場合は許可しない。Excel入力値は `parsed_facility_location_id` に保持し、解決済みIDとは別管理する',
      ...
      '`isConfirmed=true` の場合は `facility_location_id IS NOT NULL` かつ対象施設に属することを必須とし、未解決行は `LOCATION_UNRESOLVED` として確定を拒否する',
    ...
    ResponseSubtables = @(
      @{
        ...
        Rows = @(
          @('assetImportRowId', 'int64', '✓', '資産インポート行ID'),
          @('parsedFacilityLocationId', 'string', '-', '施設ロケーションID（Excel入力原文）'),
          @('facilityLocationId', 'int64|null', '-', '解決済み施設ロケーションID。未解決は null'),
          @('facilityLocation', 'FacilityLocationSummary|null', '✓', '解決済み施設ロケーションの表示値。未解決は null'),
    ...
    StatusRows = @(
      ...
      @('400', '入力不正、親子不整合、施設ロケーション不正または未解決状態での確定', 'ErrorResponse'),
      @('401', '未認証', 'ErrorResponse'),
      @('403', '通常アカウントで作業対象施設に対する実効 `survey_ledger_matching` なし、共有システム管理者で作業対象施設が削除済み、または対象施設不一致', 'ErrorResponse'),
  ...
  @{
    ...
    ProcessingLines = @(
      ...
      '指定行の `facility_location_id` が NULL でないこと、および各ロケーションが対象ジョブの `facility_id` に属する有効な `facility_locations` であることを検証する。未解決行を含む場合は一括確定せず、行IDを `details` に返却する',
    ...
    StatusRows = @(
      ...
      @('400', '入力不正、未解決または施設外のロケーションを含む確定不可行', 'ErrorResponse'),
      @('401', '未認証', 'ErrorResponse'),
      @('403', '通常アカウントで作業対象施設に対する実効 `survey_ledger_matching` なし、共有システム管理者で作業対象施設が削除済み、または対象施設不一致', 'ErrorResponse'),
  ...
  @{
    ...
    ParametersRows = @(
      @('assetImportJobId', 'query', 'int64', '✓', '対象ジョブID'),
      @('facilityLocationId', 'query', 'int64', '-', '解決済み施設ロケーションID条件'),
    ...
    ProcessingLines = @(
      ...
      '現在の絞り込み条件に一致する取込結果を `突き合わせ結果` シートとして出力する。画面表示列には台帳原本値の `品目名（元）` / `メーカー名（元）` / `型式（元）` と解決済み施設ロケーションを含め、AI推薦と現在の選択結果にはマスタ側の品目/メーカー/型式を含める。再取込用の `assetImportJobId` / `assetImportRowId` / `rowNo` と、編集対象の `selectedShipAssetMasterId` も含める。AI分類結果は内部制御値のため出力列に含めない',
  ...
  @{
    ...
    Overview = '`/asset-matching/export` で出力した突き合わせ結果 Excel を受け取り、ローカルExcelで編集された SHIP 資産マスタ紐づけを `asset_import_rows` の選択結果へ反映する。固定資産台帳取込Excelの再取込ではないため、原本の取込値、正式29カラム、施設ロケーション、AI分類、AI推薦、確定状態は更新しない。'
    Method = 'POST'
    ...
    ProcessingLines = @(
      ...
      '正式取込対象29カラム、`raw_data_json`、`parsed_*`、`facility_location_id`、`ai_*`、`suggested_*`、`suggested_ship_asset_master_id`、`suggested_score`、`suggested_similarity_source`、`suggested_source_asset_ledger_id`、`is_confirmed`、`confirmed_by_user_id`、`confirmed_at` は更新しない',
  ...
  @{
    ...
    ProcessingLines = @(
      '対象ジョブの `facility_id` が Bearer トークン上の作業対象施設IDと一致し、`facilities.deleted_at IS NULL` の未削除施設であることを検証する',
       '対象ジョブが `READY_FOR_MATCHING` であり、未確定件数（`remainingRows`）が 0 件であることを検証する。行確定時に施設ロケーション解決済みを必須としているため、未確定件数 0 件はロケーション未解決行がないことも意味する',
      ...
      '完了時に `asset_import_jobs.finished_at` を更新し、`remainingRows=0` を返却する',
       '本APIは取込・マスタ突き合わせジョブの完了のみを扱い、`asset_data_matching_items` または `asset_ledgers` の作成・更新は行わない。正式取込対象29カラムの統合と原本資産への反映はデータ突合・原本確定APIで行う'
    )
    ExtraTables = @(
      @{
        Title = '永続化マッピング'
...
@{
  ...
  CoverVersionText = '1.3'
  RevisionVersionText = '1.3'
  RevisionDateText = '2026/7/21'
  RevisionSummaryText = '固定資産台帳29カラム、施設ロケーション解決、見積金額・税項目のDB整合、原本DTO名を反映'
  Sections = @(
    @{ Type = 'Heading1'; Text = '第1章 概要' },
    @{ Type = 'Heading2'; Text = '本書の目的' },
    @{ Type = 'Paragraph'; Text = '本書は、12. 資産台帳取込画面（`/asset-import`）および 13. 資産台帳とマスタの突き合わせ画面（`/asset-matching`）で利用する API の設計を定義する。資産台帳ファイルのアップロード、正式取込対象29カラム（A～AC）の保持、施設ロケーションIDの解決、取込ジョブ管理、失敗ジョブの再取込、台帳原本値を入力とする PoC2 保存済み LUKE Model A によるAI分類、ABC行のみのAI推薦付きマスタ突き合わせ、突き合わせ結果Excel 出力・取込、突き合わせ完了までを対象とする。見積金額・税項目は購入金額へ変換せず、固定資産台帳取込では値引き文字列判定を行わない。' },
