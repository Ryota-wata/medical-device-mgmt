  ...
  New-EndpointBlock `
    ...
    -RequestSubtables @(
      @{ Title = 'assets要素（AssetRegistrationInput）'; Headers = @('フィールド', '型', '必須', '説明'); Rows = @(
        ...
        @('photoDocumentIds', 'int64[]', '-', '原本資産へ反映する検収写真ドキュメントID。指定時も各IDが同一RFQ、未削除RFQ写真、かつ当該個体の `orderItemId` に対応する有効な `ACCEPTANCE_PHOTO` リンクを持つことを検証する。未指定時は同リンクからAPIが再解決する'),
    ...
    -ProcessingLines @(
      ...
      '原本資産へ反映する検収写真は上記のリンクテーブル検証を通過した写真に限定し、S3オブジェクトキーの接頭辞から `orderItemId` を判定しない。',
