...
$workFacilityProcessingLine = 'Bearer トークン上の作業対象施設が `facilities.deleted_at IS NULL` で存在することを確認する'

@{
  TemplatePath = Join-Path $taniguchiRoot 'api\テンプレート\API設計書_標準テンプレート.docx'
  OutputPath = Join-Path $taniguchiRoot 'api\Fix\API設計書_施設グループ管理.docx'
  ...
  Sections = @(
    ...
    @{ Type = 'Paragraph'; Text = '本API群は共有システム管理者アカウント専用の特権操作として扱う。API 実行時は Bearer トークンが有効であること、認証コンテキストが共有システム管理者アカウント（`users.account_type=''SYSTEM_ADMIN''`）であること、および Bearer トークン上の作業対象施設が存在し未削除であることを確認する。通常アカウント向けの施設割当や部署/役職別の機能割当、作業対象施設に対する実効 `feature_code` 判定は行わない。' },
    @{ Type = 'Table'; Headers = @('管理単位名', '管理ラベル', '対象処理'); Rows = @(
    ...
    @{ Type = 'EndpointBlocks'; Items = @(
      @{
        ...
        PermissionLines = $systemAdminPermissionLines
        ProcessingLines = @(
      ...
      @{
        ...
        StatusRows = @(
          ...
          @('401', '未認証', 'ErrorResponse'),
          @('403', '共有システム管理者アカウントではない', 'ErrorResponse'),
          @('404', '作業対象施設が存在しない/削除済み、または対象グループが存在しない', 'ErrorResponse'),
          @('409', '有効グループ名が重複している', 'ErrorResponse'),
          @('500', 'サーバー内部エラー', 'ErrorResponse')
        )
      },
      @{
        ...
        StatusRows = @(
          ...
          @('401', '未認証', 'ErrorResponse'),
          @('403', '共有システム管理者アカウントではない', 'ErrorResponse'),
          @('404', '作業対象施設が存在しない/削除済み、または対象グループが存在しない/既に無効', 'ErrorResponse'),
          @('500', 'サーバー内部エラー', 'ErrorResponse')
        )
      },
      @{
        ...
        StatusRows = @(
          ...
          @('401', '未認証', 'ErrorResponse'),
          @('403', '共有システム管理者アカウントではない', 'ErrorResponse'),
          @('404', '作業対象施設が存在しない/削除済み、または groupId 指定時に対象グループが存在しない', 'ErrorResponse'),
          @('500', 'サーバー内部エラー', 'ErrorResponse')
        )
      },
      @{
        ...
        StatusRows = @(
          ...
          @('401', '未認証', 'ErrorResponse'),
          @('403', '共有システム管理者アカウントではない', 'ErrorResponse'),
          @('404', '作業対象施設が存在しない/削除済み、または対象グループ/施設が存在しない', 'ErrorResponse'),
      ...
      @{
        ...
        StatusRows = @(
          ...
          @('401', '未認証', 'ErrorResponse'),
          @('403', '共有システム管理者アカウントではない', 'ErrorResponse'),
          @('404', '作業対象施設が存在しない/削除済み、または対象グループ/施設/所属行が存在しない', 'ErrorResponse'),
          @('500', 'サーバー内部エラー', 'ErrorResponse')
        )
      },
      @{
        ...
        StatusRows = @(
          ...
          @('401', '未認証', 'ErrorResponse'),
          @('403', '共有システム管理者アカウントではない', 'ErrorResponse'),
          @('404', '作業対象施設が存在しない/削除済み、または対象グループ/公開元施設が存在しない', 'ErrorResponse'),
