param(
  ...
  [string]$OutputPath = 'C:\Projects\mock\medical-device-mgmt\taniguchi\api\作成済み\API設計書_資産台帳取込.docx',
  [string]$ScreenLabel = '資産台帳取込',
...
$apiListRows = @(
  ...
  @('突き合わせ一覧取得', 'GET', '/asset-matching/rows', '取込結果一覧、AI推薦、確定済み選択値を取得する', '要'),
  @('突き合わせ候補取得', 'GET', '/asset-matching/master-options', 'Category/分類/品目/メーカー/型式の検索候補を取得する', '要'),
  @('突き合わせ行更新', 'PUT', '/asset-matching/rows/{assetImportRowId}', '行単位で選択マスタIDや確定フラグを更新する', '要'),
...
$endpointSpecs = @(
  @{
    ...
    PermissionLines = @(
      ...
      '現行権限マトリクス上の想定ロールは `admin` / `consultant` / `office_admin` である'
    )
    ProcessingLines = @(
  ...
  @{
    ...
    ProcessingLines = @(
      ...
      '各行について資産マスタ/JMDNマスタとの類似度計算を行い、最良候補の `suggested_*_id` と `suggested_score` を保存する',
  ...
  @{
    ...
    Overview = '取込結果一覧、AI推薦、ユーザー確定済み選択値を取得する。部門/部署/Category/大分類/中分類/品目で絞り込み可能とする。'
    Method = 'GET'
    ...
    ParametersRows = @(
      ...
      @('selectedCategoryId', 'query', 'int64', '-', '確定済みCategoryIDの条件'),
      @('selectedLargeClassId', 'query', 'int64', '-', '確定済み大分類IDの条件'),
      @('selectedMediumClassId', 'query', 'int64', '-', '確定済み中分類IDの条件'),
      @('selectedAssetItemId', 'query', 'int64', '-', '確定済み品目IDの条件'),
    ...
    ProcessingLines = @(
      ...
      '表示値は `selected_*` / `suggested_*` の関連マスタを JOIN して導出する',
      '画面要件上ページングは定義しない'
    )
  ...
  @{
    ...
    Overview = '行単位で確定値のマスタIDを保存する。編集保存と単票確定の両方に利用する。'
    Method = 'PUT'
    ...
    RequestRows = @(
      @('selectedCategoryId', 'int64', '-', '選択CategoryID'),
      @('selectedLargeClassId', 'int64', '-', '選択大分類ID'),
      @('selectedMediumClassId', 'int64', '-', '選択中分類ID'),
      @('selectedAssetItemId', 'int64', '-', '選択品目ID'),
      @('selectedManufacturerId', 'int64', '-', '選択メーカーID'),
      @('selectedModelId', 'int64', '-', '選択型式ID'),
    ...
    ProcessingLines = @(
      '`selected_*` はマスタIDで保存し、表示名はレスポンス時に JOIN で導出する',
      '親は子を兼ねる前提で、下位階層IDが指定された場合は必要な親階層IDを自動補完する',
      '上位階層変更時に整合しない下位階層IDはクリアする',
      '`isConfirmed=true` の場合は保存後に確定済みフラグを更新する'
    )
  ...
  @{
    ...
    ProcessingLines = @(
      '指定行の `is_confirmed` を true へ更新する',
      '業務必須を満たさない行が含まれる場合は一括失敗とし、どの行が失敗したかを `details` で返却する'
    )
  ...
  @{
    ...
    RequestRows = @(
      @('assetImportJobId', 'int64', '✓', '対象ジョブID'),
      @('forceComplete', 'boolean', '-', '未確定行が残っていても完了させる場合は true')
    )
    PermissionLines = @(
      '機能権限: `data_matching` が `W` 以上であること'
    )
    ProcessingLines = @(
      '通常は未確定件数 0 件を完了条件とする',
      '`forceComplete=true` の場合は未確定件数が残っていても確認ダイアログの後に `MATCHING_COMPLETED` へ更新する',
    ...
    StatusRows = @(
      ...
      @('403', '更新権限なし', 'ErrorResponse'),
      @('404', '対象ジョブが存在しない', 'ErrorResponse'),
      @('409', '未確定行が残っており forceComplete=false', 'ErrorResponse'),
      @('500', 'サーバー内部エラー', 'ErrorResponse')
    )
  }
)
...
Add-Paragraph -Selection $selection -Text '本書は、12. 資産台帳取込画面（`/asset-import`）および 13. 資産台帳とマスタの突き合わせ画面（`/asset-matching`）で利用する API の設計を定義する。資産台帳ファイルのアップロード、取込ジョブ管理、AI 推薦付きのマスタ突き合わせ、Excel 出力、突き合わせ完了までを対象とする。' -Style $wdStyleNormal

Add-Paragraph -Selection $selection -Text '対象画面' -Style $wdStyleHeading2
Add-Table -Document $doc -Selection $selection -Headers @('画面', 'URL', '主機能') -Rows @(
...
Add-Paragraph -Selection $selection -Text '利用データ' -Style $wdStyleHeading2
Add-Table -Document $doc -Selection $selection -Headers @('テーブル', '利用内容', '主な項目') -Rows @(
  @('asset_import_jobs', '取込ジョブの作成、状態管理、件数表示', 'asset_import_job_id, facility_id, import_type, file_name, status'),
  @('asset_import_rows', '取込行の正本、AI推薦、確定値、確定フラグ', 'parsed_*, suggested_*_id, selected_*_id, is_confirmed'),
  @('facilities', '選択施設の解決', 'facility_id, facility_name'),
  @('users', '取込実行ユーザーの記録', 'user_id'),
...
Add-Paragraph -Selection $selection -Text 'ジョブ状態遷移' -Style $wdStyleHeading2
...
Add-Paragraph -Selection $selection -Text '資産台帳取込画面は機能コード `asset_import`、突き合わせ画面は機能コード `data_matching` を用いる前提とする。現行権限マトリクスでは両者とも `admin=F`、`consultant=W`、`office_admin=W` である。' -Style $wdStyleNormal
Add-Table -Document $doc -Selection $selection -Headers @('処理', '機能権限コード', '必要レベル', '想定ロール') -Rows @(
  @('取込画面コンテキスト取得 / ジョブ状態取得', 'asset_import', 'W以上', 'admin, consultant, office_admin'),
  @('ファイルアップロード / ジョブ削除', 'asset_import', 'W以上', 'admin, consultant, office_admin'),
  @('突き合わせ画面コンテキスト取得 / 一覧取得 / 候補取得 / Excel出力', 'data_matching', 'W以上', 'admin, consultant, office_admin'),
  @('行更新 / 一括確定 / 突き合わせ完了', 'data_matching', 'W以上', 'admin, consultant, office_admin')
)
...
Add-Paragraph -Selection $selection -Text '資産台帳取込 / 資産台帳とマスタの突き合わせ' -Style $wdStyleHeading2
Add-Table -Document $doc -Selection $selection -Headers @('機能名', 'Method', 'Path', '概要', '認証') -Rows $apiListRows

Add-Paragraph -Selection $selection -Text '第5章 資産台帳取込・突き合わせ機能設計' -Style $wdStyleHeading1
...
Add-Paragraph -Selection $selection -Text '施設単位運用ルール' -Style $wdStyleHeading2
Add-BulletsAsParagraphs -Selection $selection -Items @(
...
Add-Paragraph -Selection $selection -Text '突き合わせ保存ルール' -Style $wdStyleHeading2
Add-BulletsAsParagraphs -Selection $selection -Items @(
  '`selected_*` / `suggested_*` はマスタIDを正本とし、表示名は関連マスタ JOIN で導出する',
  '下位階層IDが入る場合は必要な親階層IDも同時に保持する',
  '上位階層変更時は整合しない下位階層IDをクリアする',
  '行保存は即時に `asset_import_rows` へ反映し、別途一時保存専用テーブルは設けない前提とする'
)

Add-Paragraph -Selection $selection -Text '完了・削除ルール' -Style $wdStyleHeading2
Add-BulletsAsParagraphs -Selection $selection -Items @(
  '突き合わせ完了時は `asset_import_jobs.status` を `MATCHING_COMPLETED` へ更新する',
...
Add-BulletsAsParagraphs -Selection $selection -Items @(
  ...
  '現行モックの AI推薦・確定値は表示文字列ベースで保持しているが、要件/DB設計では `selected_*_id` / `suggested_*_id` を正本とする',
  '戻る操作時の「一時保存して戻る」機能はモックに存在するが、要件では各行保存で反映される前提のため、専用一時保存APIを設けるかはクライアント最終確認待ちである',
  '未確定行が残る状態での突き合わせ完了可否は要件に明記がなく、現行モックは確認後に完了できるため、本設計では `forceComplete` を暫定定義した'
)
...
Add-Paragraph -Selection $selection -Text '突き合わせ運用方針' -Style $wdStyleHeading2
Add-BulletsAsParagraphs -Selection $selection -Items @(
  '確定値はマスタIDベースで保存し、表示名は常に最新マスタから導出する',
